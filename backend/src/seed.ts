// Seed script — creates demo data using the query wrapper
...
export function runSeed(): void {
  ...
  db.prepare('INSERT INTO vessels (id, organization_id, name, imo_number, mmsi, flag_state, port_of_registry, length_m, beam_m, draft_m, gross_tonnage, build_year, builder, max_crew, max_guests, current_location) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)')
  ...
  for (const crew of crewData) {
    ...
    db.prepare('INSERT INTO crew_members (id, vessel_id, first_name, last_name, email, nationality, position, department, join_date, contract_end_date, salary, salary_currency, passport_number, passport_expiry) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)')
      .run(crewId, vesselId, crew.first_name, crew.last_name, crew.email, crew.nationality, crew.position, crew.department, crew.join_date, crew.contract_end_date, crew.salary, crew.salary_currency, crew.passport_number, crew.passport_expiry);
  }

  // Fetch crew for cert seeding
  const crewMembers = db.prepare('SELECT id, position FROM crew_members WHERE vessel_id = ?').all(vesselId) as any[];

  const certTemplates: Record<string, any[]> = {
    'Captain': [
  ...
  for (const member of crewMembers) {
    ...
    for (const ct of templates) {
      ...
      db.prepare('INSERT INTO certifications (id, crew_member_id, cert_type, cert_name, issuing_authority, issue_date, expiry_date) VALUES (?,?,?,?,?,?,?)')
  ...
  for (const member of crewMembers) {
    ...
    db.prepare('INSERT INTO rotations (id, crew_member_id, rotation_pattern, days_on, days_off) VALUES (?,?,?,?,?)')
