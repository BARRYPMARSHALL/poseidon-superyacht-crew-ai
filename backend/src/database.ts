...
export async function initializeDatabase(): Promise<void> {
  ...
  const tables = [
    ...
    `CREATE TABLE IF NOT EXISTS leads (id TEXT PRIMARY KEY, company_name TEXT NOT NULL, contact_name TEXT, email TEXT, phone TEXT, website TEXT, source TEXT, status TEXT DEFAULT 'new', industry TEXT, notes TEXT, last_contacted TEXT, email_count INTEGER DEFAULT 0, reply_count INTEGER DEFAULT 0, created_at TEXT NOT NULL DEFAULT (datetime('now')), updated_at TEXT NOT NULL DEFAULT (datetime('now')))`,
    `CREATE TABLE IF NOT EXISTS telegram_chats (id TEXT PRIMARY KEY, chat_id TEXT UNIQUE NOT NULL, user_id TEXT, vessel_id TEXT, chat_type TEXT DEFAULT 'user', is_active INTEGER DEFAULT 1, created_at TEXT NOT NULL DEFAULT (datetime('now')))`,
    `CREATE TABLE IF NOT EXISTS ad_campaigns (id TEXT PRIMARY KEY, platform TEXT NOT NULL, campaign_name TEXT NOT NULL, status TEXT DEFAULT 'draft', daily_budget_cents INTEGER DEFAULT 1000, total_spend_cents INTEGER DEFAULT 0, impressions INTEGER DEFAULT 0, clicks INTEGER DEFAULT 0, conversions INTEGER DEFAULT 0, meta_campaign_id TEXT, start_date TEXT, end_date TEXT, created_at TEXT NOT NULL DEFAULT (datetime('now')), updated_at TEXT NOT NULL DEFAULT (datetime('now')))`,
    `CREATE TABLE IF NOT EXISTS workflows (id TEXT PRIMARY KEY, vessel_id TEXT, name TEXT NOT NULL, domain TEXT NOT NULL, steps TEXT NOT NULL, triggers TEXT, inputs TEXT, outputs TEXT, exceptions TEXT, tools_used TEXT, frequency TEXT, owner TEXT, estimated_hours_monthly REAL, ai_readiness INTEGER DEFAULT 0, is_documented INTEGER DEFAULT 0, notes TEXT, created_at TEXT NOT NULL DEFAULT (datetime('now')), updated_at TEXT NOT NULL DEFAULT (datetime('now')))`,
    `CREATE TABLE IF NOT EXISTS sensing_events (id TEXT PRIMARY KEY, vessel_id TEXT, category TEXT NOT NULL, source TEXT, title TEXT NOT NULL, description TEXT, severity TEXT DEFAULT 'info', url TEXT, detected_at TEXT NOT NULL DEFAULT (datetime('now')), is_actionable INTEGER DEFAULT 0, action_taken TEXT, resolved_at TEXT, created_at TEXT NOT NULL DEFAULT (datetime('now')))`,
    `CREATE TABLE IF NOT EXISTS agent_outcomes (id TEXT PRIMARY KEY, vessel_id TEXT, agent_name TEXT NOT NULL, action_type TEXT NOT NULL, input_summary TEXT, output_summary TEXT, outcome TEXT DEFAULT 'unknown', success_score INTEGER, lessons TEXT, improvement_suggestions TEXT, reviewed INTEGER DEFAULT 0, created_at TEXT NOT NULL DEFAULT (datetime('now')))`,
    `CREATE TABLE IF NOT EXISTS agent_checkpoints (id TEXT PRIMARY KEY, vessel_id TEXT, agent_name TEXT NOT NULL, snapshot_type TEXT NOT NULL, snapshot_data TEXT NOT NULL, checksum TEXT, parent_checkpoint_id TEXT, status TEXT DEFAULT 'active', rolled_back_at TEXT, created_at TEXT NOT NULL DEFAULT (datetime('now')))`,
    `CREATE TABLE IF NOT EXISTS human_review_queue (id TEXT PRIMARY KEY, vessel_id TEXT, agent_name TEXT NOT NULL, action_type TEXT NOT NULL, description TEXT NOT NULL, context_data TEXT, status TEXT DEFAULT 'pending', reviewed_by TEXT, reviewed_at TEXT, reviewer_notes TEXT, checkpoint_id TEXT, created_at TEXT NOT NULL DEFAULT (datetime('now')))`,
  ];
